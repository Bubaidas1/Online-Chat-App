...
import 'package:chat/core/models/chat.dart';
...
class ChatService {
  ...
  final StreamController<List<ChatSet>> _chatController =
      StreamController<List<ChatSet>>.broadcast();

  final StreamController<List<Chat>> _personalChatController =
  ...
  Stream getActiveChats({String userId}) {
    _chatCollection
        ...
        .snapshots()
        .listen((activeChatSnapshots) {
      if (activeChatSnapshots.docs.isNotEmpty) {
        var activeChats = activeChatSnapshots.docs
            .map((snapshots) => ChatSet.fromMap(snapshots.data()))
            .toList();
  ...
  Stream loadChats({String chatId}) {
    _chatCollection
      ...
      if (personalChatSnapshots.docs.isNotEmpty) {
        var chats = personalChatSnapshots.docs
            .map((snapshot) => Chat.fromMap(snapshot.data()))
            .toList();
