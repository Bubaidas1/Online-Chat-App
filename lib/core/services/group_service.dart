...
class GroupService {
  ...
  final StreamController<List<Chat>> _groupChatController =
  ...
  Stream getGroups() {
    ...
    _groupCollection.snapshots().listen((groupSnapshots) {
      ...
      if (groupSnapshots.docs.isNotEmpty) {
        var groups = groupSnapshots.docs
            .map((snapshot) => Group.fromMap(snapshot.data()))
            .toList();
  ...
  Stream getGroupChats({String groupId}) {
    ...
    _groupCollection
      ...
      if (groupChatSnapshots.docs.isNotEmpty) {
        ...
        var chats = groupChatSnapshots.docs
