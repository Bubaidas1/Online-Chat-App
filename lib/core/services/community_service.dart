...
class CommunityService {
  ...
  final StreamController<List<Community>> _communityController =
      StreamController<List<Community>>.broadcast();

  final StreamController<List<Chat>> _communityChats =
  ...
  Stream getCommunities() {
    ...
    _communityCollection.snapshots().listen((communitySnapshots) {
      ...
      if (communitySnapshots.docs.isNotEmpty) {
        var community = communitySnapshots.docs
            .map((snapshot) => Community.fromMap(snapshot.data()))
            .toList();
  ...
  Stream getCommunityChats({String communityId}) {
    ...
    _communityCollection
      ...
      if (communityChatSnapshots.docs.isNotEmpty) {
        var chats = communityChatSnapshots.docs
